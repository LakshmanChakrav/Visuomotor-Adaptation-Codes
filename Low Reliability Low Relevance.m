...
for N=1:100
    ...
    TargetRect(N,:) = [PointTx-PointTW/2 , (PointTy-PointTH/2-3*15) ,...
        PointTx+PointTW/2 , (PointTy+PointTH/2+3*15)];
end
...
%     b = floor(15*randn(1,25));
    radius = floor(14*abs(randn(1,64)));
    JmpDis = 23;
    ticbase = [];    
    while toc(ticStore2(1)) < RecordingTime
        ...
        if catchTrials(find(catchTrials==N))==N
            ...
            if CursorJumpDist~=0
                IsCursorJump = 1;
                yCursor = yCursor - JmpDis; %Cursor Jump distance with spread of 0.5 cm SD
                if startCursorDist >= CursorAppLength
                   ...
                   if (toc(ticbase(1)) < 0.1)
                        for i = 1:64
                            myRect(1,i) = xCursor - CursorWidth/4 + radius(1,i)*cos(i*pi/32);%+ a(1,i);
                            myRect(2,i) = yCursor - CursorHeight/4 + radius(1,i)*sin(i*pi/32);%+ b(1,i);
                            myRect(3,i) = xCursor + CursorWidth/4 + radius(1,i)*cos(i*pi/32);%+ a(1,i);
                            myRect(4,i) = yCursor + CursorHeight/4 + radius(1,i)*sin(i*pi/32);%+ b(1,i);
                        end
                        Screen('FillOval',wPtr,CursorColor,myRect);
                   else
                   end
            ...
            Screen('FillOval',wPtr,StartColor,StartRect(N,:));
            Screen('FillRect',wPtr,TargetColor,TargetRect(N,:));
            Screen('Flip',wPtr);
        end
        %%
        %------------------Calculations-------------------------
        ...
        cursor_targetDist(N) = abs(yCursor-Ty);
    ...
    TargetRadius = 45;%abs(TargetRect(N,1) - TargetRect(N,3))/2;
